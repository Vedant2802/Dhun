    ...
    <>
      ...
      <div className={styles.popupBox}>
           ...
           <div className={styles.popuppart}>
              ...
              <div >
                    <div onClick={() => setEmotionOptionOpen(!emotionOptionopen)} className={styles.popupdropdown}> 
                      <p>{emotion}</p>
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <path fillRule="evenodd" clip-rule="evenodd" d="M10.024 12.5865L4.31205 6.02655C4.08836 5.76965 3.72569 5.76965 3.502 6.02655C3.27831 6.28345 3.27831 6.69996 3.502 6.95686L9.56548 13.9205C9.59402 13.9533 9.62481 13.9818 9.65729 14.0063C9.88232 14.2297 10.2228 14.2188 10.4365 13.9733L16.5 7.00972C16.7237 6.75282 16.7237 6.33631 16.5 6.07941C16.2763 5.82251 15.9137 5.82251 15.69 6.07941L10.024 12.5865Z" fill="#ADADAD"/>
                      </svg>
                    </div>
                  {emotionOptionopen &&   <ul className={styles.popupdropdownoptions}>
                      {EMOTION.map((item,index)=> (
                        <li className={styles.popupdropdownoptionsingle} onClick={() => {setEmotionOptionOpen(false); setEmotion(item); }} id={String(index)}>{item}</li>
                      ))}
                    </ul> }
              </div>
           </div>
           <div className={styles.popuppart}>
              <p>Instruments</p>
              <div >
                    <div onClick={() => setIntrumentOptionsOpen(!instrumentOptionsOpen)} className={styles.popupdropdown}>
                      <p>{instruments}</p>
                      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <path fillRule="evenodd" clip-rule="evenodd" d="M10.024 12.5865L4.31205 6.02655C4.08836 5.76965 3.72569 5.76965 3.502 6.02655C3.27831 6.28345 3.27831 6.69996 3.502 6.95686L9.56548 13.9205C9.59402 13.9533 9.62481 13.9818 9.65729 14.0063C9.88232 14.2297 10.2228 14.2188 10.4365 13.9733L16.5 7.00972C16.7237 6.75282 16.7237 6.33631 16.5 6.07941C16.2763 5.82251 15.9137 5.82251 15.69 6.07941L10.024 12.5865Z" fill="#ADADAD"/>
                      </svg>
                    </div>
                    {instrumentOptionsOpen &&   <ul className={styles.popupdropdownoptions}>
                      {INSTRUMENTS.map((item,index)=> (
                        <li className={styles.popupdropdownoptionsingle} onClick={() => {setIntrumentOptionsOpen(false); setInstruments(item.label) }} id={String(index)}>{item.label}</li>
                      ))}
                    </ul> }
              </div>
           </div>
           <div className={styles.popuppart}>
              ...
              <div onClick={() => setGenreOptionsOpen(!genreOptionsOpen)}>
                  <div className={styles.popupdropdown}>
                    ...
                    {genreOptionsOpen &&   <ul className={styles.popupdropdownoptions}>
                      {GENRE.map((item,index)=> (
                        <li className={styles.popupdropdownoptionsingle} onClick={() => {setGenreOptionsOpen(false); SetGenre(item) }} id={String(index)}>{item}</li>
                      ))}
                    </ul> }
              </div>
           </div>
           <div className={styles.popuppart}>
              ...
              <div className={styles.tempoOptions}>
                  <div onClick={() => setTempo("High")} className={`${styles.tempOption} ${tempo === 'High' && `${styles.tempSelected}`}`}>High</div>
                  <div onClick={() => setTempo("Medium")} className={`${styles.tempOption} ${tempo === 'Medium' && `${styles.tempSelected}`}`}>Medium</div>
                  <div onClick={() => setTempo("Low")} className={`${styles.tempOption} ${tempo === 'Low' && `${styles.tempSelected}`}`}>Low</div>
              </div>
           </div>
